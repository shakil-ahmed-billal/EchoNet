...
import { deletePost, updatePost, toggleReaction, toggleSave } from "@/services/posts-client.service";
...
export function PostCard({ post }: PostCardProps) {
  ...
  const reactionMutation = useMutation({
    mutationFn: async (type: string) => {
      return toggleReaction(post.id, type);
    },
    onMutate: (type) => {
      const isRemoving = currentReactionType === type;
      setCurrentReactionType(isRemoving ? null : type);
      setIsLiked(isRemoving ? false : true);
      setLikesCount(prev => isRemoving ? prev - 1 : (currentReactionType ? prev : prev + 1));
    },
    onSuccess: (data) => {
        // Handle actual result if needed
    },
    onError: () => {
      setIsLiked(post.isLiked);
      setCurrentReactionType(post.currentReactionType || (post.isLiked ? "LIKE" : null));
      setLikesCount(post._count.reactions);
    }
  });
    ...
    <div className="bg-card/40 border-y sm:border border-border/40 sm:rounded-3xl shadow-sm flex flex-col group transition-all duration-500 hover:bg-card/60 hover:border-border/60 mb-4 sm:mb-6 overflow-hidden">
      ...
      <div className="p-3 sm:p-5 flex items-center justify-between">
        ...
        <div className="flex items-center gap-1">
          ...
          <Button 
            variant="ghost" 
            size="icon" 
            className="h-9 w-9 rounded-full text-muted-foreground hover:bg-muted/80 hover:text-foreground"
            onClick={() => toast.info("Post hidden")}
          >
             <X className="h-4 w-4" />
          </Button>
        </div>
      ...
      <div className="px-5 py-1.5 border-t border-border/10 flex items-center justify-between bg-muted/5">
        <div className="flex gap-1 flex-1">
          <ReactionPicker onReact={(type) => reactionMutation.mutate(type)} currentReaction={currentReactionType || undefined}>
            <button 
                className={cn(
          ...
          >
            ...
            <button 
              ...
              className="flex-1 flex items-center justify-center gap-2 py-2.5 px-3 rounded-xl transition-all group/share text-muted-foreground hover:bg-muted/80 hover:text-primary"
            >
            ...
            >
              <Bookmark className={cn("h-4.5 w-4.5 transition-transform group-hover/bookmark:scale-125 duration-300", isSaved && "fill-current")} />
