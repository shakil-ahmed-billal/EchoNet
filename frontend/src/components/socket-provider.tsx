...
export const SocketProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  useEffect(() => {
    ...
    // We use "/" so that the socket explicitly connects to the Next.js frontend server.
    // This perfectly mirrors how `api-client.ts` hits `/api/v1`.
    // Next.js will then proxy this request to the backend via the rewrites in `next.config.ts`.
    // This ensures cookies are reliably sent to the same-site domain.
