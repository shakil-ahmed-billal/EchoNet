...
export async function toggleReaction(postId: string, type: string): Promise<any> {
    return await httpClient.post<any>("/reactions", { postId, type });
}

export async function toggleSave(postId: string, isSaved: boolean): Promise<any> {
    if (isSaved) {
        return await httpClient.delete<any>(`/saved-posts/${postId}`);
    } else {
        return await httpClient.post<any>(`/saved-posts/${postId}`, {});
    }
}
