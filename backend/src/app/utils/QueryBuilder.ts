...
> {
      ...
      const searchConditions: Record<string, unknown>[] = searchableFields.map((field) => {
        if (field.includes('.')) {
          ...
          if (parts.length === 2) {
            ...
            const stringFilter: PrismaStringFilter = { contains: searchTerm, mode: 'insensitive' };
            return { [relation]: { [nestedField]: stringFilter } };
          } else if (parts.length === 3) {
            const [relation, nestedRelation, nestedField] = parts;
            const stringFilter: PrismaStringFilter = { contains: searchTerm, mode: 'insensitive' };
            return { [relation]: { some: { [nestedRelation]: { [nestedField]: stringFilter } } } };
          }
        }
        const stringFilter: PrismaStringFilter = { contains: searchTerm, mode: 'insensitive' };
  ...
  filter(): this {
    ...
    Object.keys(filterParams).forEach((key) => {
      ...
      if (key.includes('.')) {
        ...
        if (parts.length === 2) {
          const [relation, nestedField] = parts;
          if (!queryWhere[relation]) { queryWhere[relation] = {}; countQueryWhere[relation] = {}; }
        ...
        if (parts.length === 3) {
          const [relation, nestedRelation, nestedField] = parts;
          if (!queryWhere[relation]) { queryWhere[relation] = { some: {} }; countQueryWhere[relation] = { some: {} }; }
          const qSome = (queryWhere[relation] as any).some as Record<string, unknown>;
          const cSome = (countQueryWhere[relation] as any).some as Record<string, unknown>;
          if (!qSome[nestedRelation]) { qSome[nestedRelation] = {}; cSome[nestedRelation] = {}; }
          (qSome[nestedRelation] as Record<string, unknown>)[nestedField] = this.parseFilterValue(value);
          (cSome[nestedRelation] as Record<string, unknown>)[nestedField] = this.parseFilterValue(value);
          return;
        }
      }
  ...
  paginate(): this {
    ...
    const limit = Math.min(Number(this.queryParams.limit) || 10, 100); // max 100
  ...
  private parseRangeFilter(
    ...
    Object.keys(value).forEach((operator) => {
      ...
      switch (operator) {
        ...
        case 'equals': case 'not': case 'contains':
