...
router.get('/stats', auth(Role.ADMIN, Role.MODERATOR), redisCache('admin_stats', 30), AdminControllers.getDashboardStats);
router.get('/users', auth(Role.ADMIN, Role.MODERATOR), redisCache('users', 60), AdminControllers.getAllUsers);
...
// Moderation Routes
router.get('/posts', auth(Role.ADMIN, Role.MODERATOR), redisCache('posts', 60), AdminControllers.getAllPosts);
...
router.get('/stories', auth(Role.ADMIN, Role.MODERATOR), redisCache('stories', 60), AdminControllers.getAllStories);
router.delete('/stories/:id', auth(Role.ADMIN, Role.MODERATOR), AdminControllers.deleteStory);

router.get('/products', auth(Role.ADMIN, Role.MODERATOR), redisCache('products', 60), AdminControllers.getAllProducts);
...
router.get('/properties', auth(Role.ADMIN, Role.MODERATOR), redisCache('properties', 60), AdminControllers.getAllProperties);
