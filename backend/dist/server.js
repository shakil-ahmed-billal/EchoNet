...
var getAllUsers = async (query, currentUserId) => {
  ...
  const result = await prisma_default.user.findMany({
    where: Object.keys(filters).length > 0 ? filters : void 0,
...
var auth = betterAuth({
  ...
  advanced: {
    useSecureCookies: config_default.env === "production",
    cookies: {
      sessionToken: {
        attributes: {
...
var deletePost2 = catchAsync_default(async (req, res) => {
  const id = req.params.id;
  const authorId = req.user?.id;
  const isAdmin = req.user?.role === import_client3.Role.ADMIN;
...
router3.post(
  "/",
  auth_default(import_client4.Role.USER, import_client4.Role.ADMIN, import_client4.Role.MODERATOR),
  upload.array("media", 10),
  PostControllers.createPost
);
router3.get("/", optionalAuth, PostControllers.getAllPosts);
router3.patch("/:id", auth_default(import_client4.Role.USER, import_client4.Role.ADMIN, import_client4.Role.MODERATOR), PostControllers.updatePost);
router3.patch("/:id/status", auth_default(import_client4.Role.ADMIN, import_client4.Role.MODERATOR), PostControllers.updatePostStatus);
router3.delete("/:id", auth_default(import_client4.Role.ADMIN, import_client4.Role.MODERATOR, import_client4.Role.USER), PostControllers.deletePost);
...
router4.post("/post/:postId", auth_default(import_client5.Role.USER, import_client5.Role.ADMIN, import_client5.Role.MODERATOR), CommentControllers.createComment);
router4.get("/post/:postId", CommentControllers.getCommentsForPost);
router4.delete("/:id", auth_default(import_client5.Role.USER, import_client5.Role.ADMIN, import_client5.Role.MODERATOR), CommentControllers.deleteComment);
...
router5.post("/toggle", auth_default(import_client6.Role.USER, import_client6.Role.ADMIN, import_client6.Role.MODERATOR), LikeControllers.toggleLike);
...
var connectRedis = async () => {
  try {
    ...
    logger_default.error("Could not connect to Redis. Some features may be limited.", error);
  }
};
...
var io;
var initSocket = async (server2) => {
  logger_default.info("Initializing Socket.IO...");
  io = new SocketIOServer(server2, {
    cors: {
  ...
  if (config_default.redis_url) {
    try {
      const pubClient = createClient2({ url: config_default.redis_url });
      const subClient = pubClient.duplicate();
      await Promise.all([pubClient.connect(), subClient.connect()]);
      io.adapter(createAdapter(pubClient, subClient));
      logger_default.info("Socket.IO Redis adapter connected");
    } catch (error) {
      logger_default.error("Failed to connect Redis for Socket.IO:", error);
    }
  }
  io.on("connection", (socket) => {
    ...
    if (userId) {
      socket.join(userId);
      logger_default.info(`User ${userId} joined their private room`);
      socket.broadcast.emit("user-status", { userId, status: "online" });
      import("./prisma-WAEP42T6.js").then((m) => {
        m.prisma.groupMember.findMany({ where: { userId } }).then((groups) => {
          groups.forEach((g) => {
            socket.join(`group_${g.groupId}`);
            logger_default.info(`User ${userId} joined group_${g.groupId}`);
          });
        }).catch((err) => logger_default.error("Failed to join groups: ", err));
      });
    }
    ...
    socket.on("disconnect", () => {
      logger_default.info(`Socket disconnected: ${socket.id}`);
      if (userId) {
...
var followUser = async (followerId, followingId) => {
  ...
  await NotificationServices.createNotification({
    userId: followingId,
    type: status2 === "ACCEPTED" ? import_client7.NotificationType.FOLLOW : import_client7.NotificationType.FOLLOW_REQUEST,
...
var acceptFollow = async (receiverId, senderId) => {
  ...
  await NotificationServices.createNotification({
    userId: senderId,
    type: import_client7.NotificationType.FRIEND_ACCEPT,
...
var router6 = Router2();
router6.use(auth_default(import_client8.Role.USER, import_client8.Role.ADMIN, import_client8.Role.MODERATOR));
...
var getChatHistory2 = catchAsync_default(async (req, res) => {
  ...
  const userId = req.user.id;
  const prisma2 = await import("./prisma-WAEP42T6.js").then((m) => m.prisma);
...
router11.get("/", auth_default(import_client9.Role.USER, import_client9.Role.ADMIN, import_client9.Role.MODERATOR), StoryControllers.getStories);
router11.post("/", auth_default(import_client9.Role.USER, import_client9.Role.ADMIN, import_client9.Role.MODERATOR), upload.single("media"), StoryControllers.createStory);
router11.delete("/:id", auth_default(import_client9.Role.USER, import_client9.Role.ADMIN, import_client9.Role.MODERATOR), StoryControllers.deleteStory);
router11.post("/:id/view", auth_default(import_client9.Role.USER, import_client9.Role.ADMIN, import_client9.Role.MODERATOR), StoryControllers.viewStory);
...
var getAllProducts = async (query) => {
  ...
  const result = await prisma_default.product.findMany({
    where: filters,
    include: {
      store: {
        select: {
...
var getAllProperties = async (query) => {
  ...
  const where = {
    status: query.status || import_client10.PropertyStatus.ACTIVE,
    deletedAt: null
  };
  ...
  const [properties, total] = await Promise.all([
    prisma_default.property.findMany({
      where,
      include: {
        details: true,
    ...
    prisma_default.property.count({ where })
  ]);
  return {
    meta: {
      page: Number(page),
...
var deleteProperty = async (id, userId) => {
  ...
  return await prisma_default.property.update({
    where: { id },
    data: { deletedAt: /* @__PURE__ */ new Date(), status: import_client10.PropertyStatus.INACTIVE }
  });
};
var getMyProperties = async (userId) => {
  return await prisma_default.property.findMany({
    where: {
      ownerId: userId,
      deletedAt: null
    },
...
router22.post("/", auth_default("USER", "ADMIN", "MODERATOR"), PropertyControllers.createProperty);
...
router22.put("/:id", auth_default("USER", "ADMIN", "MODERATOR"), PropertyControllers.updateProperty);
...
var updateBookingStatus = async (id, userId, status2) => {
  ...
  if (status2 === import_client11.BookingStatus.CANCELLED && !isVisitor && !isOwner) {
    throw new Error("Not authorized to cancel this booking");
  }
  if ((status2 === import_client11.BookingStatus.CONFIRMED || status2 === import_client11.BookingStatus.DONE) && !isOwner) {
...
var getAllAgents = async (query = {}) => {
  ...
  return await prisma_default.agentProfile.findMany({
    where,
    include: {
      user: { select: { name: true, avatarUrl: true, email: true } }
    },
...
moduleRoutes.forEach((route) => {
  router26.use(route.path, route.route);
});
...
app.use(express17.urlencoded({ extended: true, limit: "10mb" }));
app.use(sanitizeRequest);
app.use(morgan("dev"));
app.use("/api/v1", routes_default);
app.get("/", (req, res) => {
  res.status(httpStatus19.OK).json({
    success: true,
...
async function bootstrap() {
  try {
    server = app_default.listen(config_default.port, () => {
      console.log(`EchoNet server is listening on port ${config_default.port}`);
    });
